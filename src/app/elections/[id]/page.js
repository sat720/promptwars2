...
import { ROUTES, ELECTION_STATUS } from '@/constants';
      ...
      {stages.map((stage, i) => (
        <div key={stage.id} className="timeline-item">
          ...
          <div>
            ...
            <div style={{ fontSize: '0.8rem', color: 'var(--text3)', marginBottom: 4 }}>{stage.date}</div>
            <div style={{ fontSize: '0.85rem', color: 'var(--text2)', lineHeight: 1.5 }}>{stage.description}</div>
          </div>
        </div>
      ))}
    </div>
...
export default function ElectionDetailPage() {
    ...
    return (
      <div className="page-wrapper flex-center" style={{ minHeight: '100vh', textAlign: 'center' }}>
        <div>
          ...
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: 12 }}>Election not found</h1>
          <Link href={ROUTES.ELECTIONS}><button className="btn btn-primary">Back to Elections</button></Link>
        </div>
      </div>
    );
  }

  const isEligible = voterData && (election.state === voterData.state || election.type === 'Lok Sabha');
  ...
  const tabs = [
    ...
    ...(isToday ? [{ key: 'booth', label: '🗳️ Vote Today' }] : []),
  ];
      ...
      <div className="container" style={{ padding: '60px 24px' }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <div className="flex gap-12" style={{ alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 10 }}>
            ...
            <h1 style={{ fontSize: 'clamp(1.4rem, 3vw, 2rem)', fontWeight: 800 }}>{election.name}</h1>
            <span className={`badge ${election.status === ELECTION_STATUS.ONGOING ? 'badge-success' : election.status === ELECTION_STATUS.UPCOMING ? 'badge-warning' : 'badge-gray'}`}>
              {election.status === ELECTION_STATUS.ONGOING ? '🔴 Voting TODAY' : election.status === ELECTION_STATUS.UPCOMING ? '🟡 Upcoming' : '⚫ Completed'}
            </span>
            {voterData && (
              <span className={`badge ${isEligible ? 'badge-success' : 'badge-gray'}`}>
                {isEligible ? '✅ You are eligible' : '❌ Not in your state'}
              </span>
            )}
          </div>
        ...
        {tab === 'overview' && (
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 24 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              <div className="card">
                <h2 style={{ fontWeight: 700, marginBottom: 16 }}>About This Election</h2>
                <p style={{ color: 'var(--text2)', lineHeight: 1.7, marginBottom: 16 }}>{election.description}</p>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  {[
                    ['What does it elect?', election.whatElected],
                    ['Who can vote?', election.whoCanVote],
                  ].map(([k, v]) => (
                    <div key={k} style={{ padding: 16, background: 'var(--bg3)', borderRadius: 10 }}>
                      <div style={{ fontSize: '0.75rem', color: 'var(--text3)', marginBottom: 6 }}>{k}</div>
                      <div style={{ fontWeight: 600, fontSize: '0.9rem', lineHeight: 1.4 }}>{v}</div>
                    </div>
                  ))}
                </div>
              </div>
              {isToday && isEligible && (
                <div className="card" style={{ background: 'rgba(34,197,94,0.05)', border: '1px solid rgba(34,197,94,0.3)' }}>
                  ...
                  <button className="btn btn-success" onClick={() => setTab('booth')} id="go-to-booth-tab-btn" style={{ background: 'linear-gradient(135deg, #22c55e, #16a34a)', color: 'white', border: 'none' }}>
                    <Navigation size={16} /> Find My Booth
                  </button>
                </div>
              )}
            </div>
            <div className="card">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div><div style={{ fontSize: '0.75rem', color: 'var(--text3)' }}>Election Type</div><div style={{ fontWeight: 600 }}>{election.type}</div></div>
                <div><div style={{ fontSize: '0.75rem', color: 'var(--text3)' }}>State</div><div style={{ fontWeight: 600 }}>{election.state}</div></div>
                <div><div style={{ fontSize: '0.75rem', color: 'var(--text3)' }}>Voting Day</div><div style={{ fontWeight: 600 }}>{new Date(election.votingDay).toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })}</div></div>
                <div><div style={{ fontSize: '0.75rem', color: 'var(--text3)' }}>Polling Hours</div><div style={{ fontWeight: 600 }}>7:00 AM – 6:00 PM</div></div>
                <div><div style={{ fontSize: '0.75rem', color: 'var(--text3)' }}>Booth Timings</div><div style={{ fontWeight: 600 }}>{election.booth.timings}</div></div>
                {voterData && <div><div style={{ fontSize: '0.75rem', color: 'var(--text3)' }}>Your Eligibility</div><div style={{ fontWeight: 600, color: isEligible ? 'var(--success)' : 'var(--text3)' }}>{isEligible ? '✅ Eligible to Vote' : '❌ Not Eligible'}</div></div>}
              </div>
            </div>
          </div>
        )}
        ...
        {tab === 'candidates' && (
          <div>
            <h2 style={{ fontWeight: 700, marginBottom: 24 }}>👥 Candidates</h2>
              ...
              {election.candidates.map((candidate, i) => (
                <div key={i} className="card" style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  ...
                  <div>
                    ...
                    <div style={{ color: 'var(--primary-light)', fontSize: '0.85rem' }}>{candidate.party}</div>
                    <div style={{ color: 'var(--text3)', fontSize: '0.8rem' }}>{candidate.constituency}</div>
                  </div>
                </div>
              ))}
            </div>
            <div className="card" style={{ marginTop: 20, background: 'rgba(99,102,241,0.05)' }}>
              <p style={{ color: 'var(--text2)', fontSize: '0.85rem' }}>
                ℹ️ These are demo candidates for educational purposes. In a real election, you can check the full candidate list on the Election Commission website.
              </p>
            </div>
          </div>
        )}
